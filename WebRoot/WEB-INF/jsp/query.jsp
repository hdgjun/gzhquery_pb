...
<script>
	...
	window.onload = function() {
		...
		$.ajax({
			type : "POST",
			url : "${pageContext.request.contextPath}/GZHQueryServlet",
			data : {"method":"queryBankInfo","login":login},
			success : function(data) {
				var resultDiv = document
				.getElementById("bankinfo");
				
				if (data != "notfound") {
					var resultStr = eval(data);
					var contentStr = "";
					var resultStrLength = resultStr.length;
					resultDiv.add(new Option("中国人民银行","0")); 
					for(var i = 0;i<resultStrLength;i++){
						resultDiv.add(new Option(resultStr[i].bankName,resultStr[i].bankNo)); 
					}
				}
			}});
	...
	function searchResult() {
		...
		if (dealNo == "") {
			...
			var bankNo = document.getElementById("bankinfo").value;
			$.ajax({	type : "POST",
						url : "${pageContext.request.contextPath}/GZHQueryServlet",
						data : {"dealNo":dealNo,"method":"queryResult","login":login,"bankNo":bankNo},
						success : function(data) {
							var resultDiv = document
...
*/
								...
								//	var num = resultStr[i].imagepath.indexOf("webapps");
									...
									contentStr = contentStr
											...
											+ "<td width='180px'><div ><img  height='20' src='data:image/png;base64,"
											+ resultStr[i].imagepath
													//.substring(num + str.length)
											+ "'/></div></td>"
											+ "<td width='80px'>"
											+ resultStr[i].mon + "</td>"
											//+"<td>"+resultStr[i].montype+"</td>"
											+ "<td width='40px'>"
											+ resultStr[i].monval + "</td>"
											+ "<td width='40px'>"
											+ resultStr[i].monver + "</td>"
											//+"<td>"+resultStr[i].trueflag+"</td>"
											//+"<td>"+resultStr[i].quanlity+"</td>"
											//+"<td>"+resultStr[i].imagesno+"</td>"
											//+"<td>"+resultStr[i].operatorid+"</td>"
											+ "<td width='80px'>"
											+ resultStr[i].operdate + "</td>"
											+ "<td width='80px'>"
											+ resultStr[i].percode + "</td>"
											//+"<td width='80px'>"+resultStr[i].pertype+"</td>"
											//+"<td>"+resultStr[i].atmcode+"</td>"
											//+"<td>"+resultStr[i].scantime+"</td>"
											+ "<td width='80px'>"
											+ resultStr[i].bankno + "</td>"
											+ "<td width='130px'>"
											+ resultStr[i].bankname + "</td>"
											+ "<td width='80px'>"
											+ resultStr[i].agencyno + "</td>"
											+ "<td width='130px'>"
											+ resultStr[i].branchname + "</td>"
								...
								var bannerDiv = document
										.getElementById("bannerDiv");
								...
								errInfo.innerHTML = "<font style='color:#999999; font-size:13px;'>人民币追踪系统帮您查找到</font>"
										+ resultStrLength
										+ "<font style='color:#999999; font-size:13px;'>条相关记录</font>";
							} else {
...
<body>
	...
	<div
		style="border: 1px solid graytext; background: url('${pageContext.request.contextPath}/style/blue/images/banner.jpg') repeat-x ; height: 600px;"
		id="bannerDiv">
		<div style="padding-top: 15px;">
			<h1>
				<font size="6" color="#FFFFFF" face="微软雅黑">&nbsp;POKA&nbsp;</font><font
					size="4" color="#FFFFFF" face="华文楷体">人民币冠字号追踪系统欢迎您</font>
			</h1>
		</div>
		...
		<form action="" id="form" name="form" method="post"
			...
			<div style='width: 100%' align="center">
				<table style="height: 70px;" align="center" cellpadding="0">
					<tr>
						<td align="center"><font size="4"><b>请输入冠字号：</b></font>&nbsp;&nbsp;
						</td>
						<td>
						<div style="float:left;font-size:14px;">
							...
							<div style="float:left;font-size:14px; margin:0; padding:0;">
								<select name="bank.name" id="bankinfo"
									style=" align:center;border: 1px solid rgb(115, 155, 192); 
									background: transparent; height: 30px;width: 100px;">
								</select>
							</div>
							</td>
						<td width="35px">
					...
					<tr>
						<td></td>
						<td>
							<div id="errorInfo" style="width: 280px; height: 40px"></div>
						</td>
						<td></td>
						<td></td>
					</tr>
